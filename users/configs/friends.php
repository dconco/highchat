<?php
    ...
    while ($follow_Data = $result->fetch_assoc()) 
    {
        ## GET THE USER INFO FROM THE FOLLOW ID
        $sqlInfo = "SELECT * FROM users WHERE (user_id = {$follow_Data['user_id']} OR user_id = {$follow_Data['friend_id']}) AND NOT user_id = {$user_id}";
        $data = $conn->query($sqlInfo)->fetch_assoc();
